<form class="dashboard-wrapper">
  <div class="title">
    <p>
      <img src="./assets/icons/dashboard/offerts.svg" alt="menu icon" width="20" height="20">Twój ofertownik
    </p>
  </div>
  <div class="dashboard-your-offers dashboard-table">
    <table>
      <tbody>
        <tr>
          <th>Nazwa</th>
          <th class="text-center">Produkty</th>
          <th class="text-center">Edytuj</th>
          <th class="text-center">Pobierz PDF</th>
          <th class="text-center">Do koszyka</th>
          <th class="text-center">Usuń</th>
        </tr>
        <tr class="spacer15"></tr>
        <tr>
          <td>
            <a href="#" class="offert-td-col">
              <strong>Oferta - 2025-01-29</strong>
              <span>Utworzona: środa, 29 stycznia 2025, 08:42</span>
            </a>
          </td>
          <td class="text-center">3</td>
          <td>
            <a href="#">
              <img src="./assets/icons/dashboard/edit.svg" alt="edit icon" width="21" height="17">
            </a>
          </td>
          <td class="download-icon">
            <a href="#">
              <img src="./assets/icons/dashboard/download.svg" alt="download icon" width="21" height="17">
            </a>
          </td>
          <td>
            <a href="#" class="cart">
              <img src="./assets/icons/dashboard/cart.svg" alt="cart icon" width="20" height="20">
            </a>
          </td>
          <td>
            <a href="#" class="delete">
              <img src="./assets/icons/dashboard/delete.svg" alt="delete icon" width="15" height="17">
            </a>
          </td>
        </tr>
        <tr class="spacer10"></tr>
        <tr>
          <td>
            <a href="#" class="offert-td-col">
              <strong>Oferta - 2025-01-29</strong>
              <span>Utworzona: środa, 29 stycznia 2025, 08:42</span>
            </a>
          </td>
          <td class="text-center">3</td>
          <td>
            <a href="#">
              <img src="./assets/icons/dashboard/edit.svg" alt="edit icon" width="21" height="17">
            </a>
          </td>
          <td class="download-icon">
            <a href="#">
              <img src="./assets/icons/dashboard/download.svg" alt="download icon" width="21" height="17">
            </a>
          </td>
          <td>
            <a href="#" class="cart">
              <img src="./assets/icons/dashboard/cart.svg" alt="cart icon" width="20" height="20">
            </a>
          </td>
          <td>
            <a href="#" class="delete">
              <img src="./assets/icons/dashboard/delete.svg" alt="delete icon" width="15" height="17">
            </a>
          </td>
        </tr>
        <tr class="spacer10"></tr>
        <tr>
          <td>
            <a href="#" class="offert-td-col">
              <strong>Oferta - 2025-01-29</strong>
              <span>Utworzona: środa, 29 stycznia 2025, 08:42</span>
            </a>
          </td>
          <td class="text-center">4</td>
          <td>
            <a href="#">
              <img src="./assets/icons/dashboard/edit.svg" alt="edit icon" width="21" height="17">
            </a>
          </td>
          <td class="download-icon">
            <a href="#">
              <img src="./assets/icons/dashboard/download.svg" alt="download icon" width="21" height="17">
            </a>
          </td>
          <td>
            <a href="#" class="cart">
              <img src="./assets/icons/dashboard/cart.svg" alt="cart icon" width="20" height="20">
            </a>
          </td>
          <td>
            <a href="#" class="delete">
              <img src="./assets/icons/dashboard/delete.svg" alt="delete icon" width="15" height="17">
            </a>
          </td>
        </tr>
        <tr class="spacer10"></tr>
        <tr>
          <td>
            <a href="#" class="offert-td-col">
              <strong>Oferta - 2025-01-29</strong>
              <span>Utworzona: środa, 29 stycznia 2025, 08:42</span>
            </a>
          </td>
          <td class="text-center">2</td>
          <td>
            <a href="#">
              <img src="./assets/icons/dashboard/edit.svg" alt="edit icon" width="21" height="17">
            </a>
          </td>
          <td class="download-icon">
            <a href="#">
              <img src="./assets/icons/dashboard/download.svg" alt="download icon" width="21" height="17">
            </a>
          </td>
          <td>
            <a href="#" class="cart">
              <img src="./assets/icons/dashboard/cart.svg" alt="cart icon" width="20" height="20">
            </a>
          </td>
          <td>
            <a href="#" class="delete">
              <img src="./assets/icons/dashboard/delete.svg" alt="delete icon" width="15" height="17">
            </a>
          </td>
        </tr>
      </tbody>
    </table>
  </div>
</form>